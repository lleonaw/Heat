c-----------------------------------------------------------------------
      ...
      parameter (lm=2000)
c     parameter (lm=20)
      real z(lm),w(lm),d(lm*lm),dt(lm*lm)
      integer fn

c -- read from keyboard 
c     write(6,*) 'input polynomial order, N:'
c     read (5,*) N
c -- read from file 
      open(fn,FILE='poly_N.txt',STATUS='OLD')
      read(fn,*) N
      close(fn) 

      nz = n+1
      if (nz.gt.lm) write(6,*) 'increase lm:',lm,nz
      if (nz.gt.lm) stop

      call zwgll (z,w,nz)

      call dgll (d,dt,z,nz,nz)

      dmax = vlmax(d,nz*nz)

c     write(6,*) n,dmax,' this is dmax'
c -- write to file 
      open(fn,FILE='Dx.txt')
c     write(fn,*) N
      do i=1,nz*nz
          write(fn,*) d(i) ! Super long txt? 
      enddo 
      close(fn) 
      write(6,*) 'Write to file done, N = ', N

      stop
      end
c-----------------------------------------------------------------------
      real function vlmax(x,n)
      real x(1), xmax
      integer n 
      xmax = x(1)
      do i=1,n
